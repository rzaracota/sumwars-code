...
HINSTANCE		  m_hInstance; // Made global by me!
...
CD3DApplication::CD3DApplication()
{
    ...
    m_bWindowed         = true;
    m_bActive           = false;
	isActiveFromWindowsMessage = false;
    m_bDeviceLost       = false;
    m_bMinimized        = false;
    m_bMaximized        = false;
    ...
    m_strWindowTitle    = _T("Demon Battle");
...
HRESULT CD3DApplication::ChooseInitialD3DSettings()
{
	...
	FILE *fp;
	fp = fopen("d3dsettings.cfg", "rb");
	if (fp)
	{
		m_bStartFullscreen=getc(fp);
		fclose(fp);
	}
    ...
    switch( uMsg )
    {
		...
		case WM_ACTIVATE:
			if (wParam==WA_INACTIVE)
				...
				isActiveFromWindowsMessage=true;
			break;
