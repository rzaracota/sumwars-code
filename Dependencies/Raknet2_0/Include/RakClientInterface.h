...
public:
	...
	*/
	virtual void InitializeSecurity( char *privKeyP, char *privKeyQ ) = 0;
	/**
	...
	*/
	virtual bool RPC( char* uniqueID, char *data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp ) = 0;
	/**
	* Calls a C function on the server that the server already
	* registered using RegisterAsRemoteProcedureCall Pass the data you
	* want to pass to that function in parameters, or 0 for no data to
