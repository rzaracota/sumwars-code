...
namespace BasicDataStructures
{
	...
	List<list_type>::List( const List& original_copy )
	{
		...
		else
		{
			...
			for ( unsigned int counter = 0L; counter < original_copy.list_size; ++counter )
	...
	void List<list_type>::insert( list_type input, unsigned int position )
	{
		...
		if ( list_size == allocation_size )
		{
			...
			// copy old array over
			for ( unsigned int counter = 0L; counter < list_size; ++counter )
	...
	template <class list_type>
	inline const unsigned int List<list_type>::size( void )
	{
		return list_size;
	}
