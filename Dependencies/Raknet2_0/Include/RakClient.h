...
public:
	...
	*/
	bool Connect( char* host, unsigned short serverPort, unsigned short clientPort, unsigned int connectionValidationInteger, int threadSleepTimer );
	...
	*/
	 ...
	 */
	bool RPC( char* uniqueID, char *data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp );
	/**
	 * Calls a C function on the server that the server already
	 * registered using RegisterAsRemoteProcedureCall Pass the data you
	 * want to pass to that function in parameters, or 0 for no data to
