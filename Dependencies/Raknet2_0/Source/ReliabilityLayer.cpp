...
static const int DEFAULT_RECEIVED_PACKETS_SIZE=256; // Divide by timeout time in seconds to get the max ave. packets per second before requiring reallocation
...
bool ReliabilityLayer::HandleSocketReceiveFromConnectedPlayer( char *buffer, int length )
{
	...
	while ( internalPacket )
	{
		...
		else
		{
			...
			if (internalPacket->packetNumber >=receivedPacketsBaseIndex &&
				(PacketNumberType)(internalPacket->packetNumber-receivedPacketsBaseIndex) < receivedPackets.size())
			{
				if (IsReceivedPacketHole(receivedPackets[internalPacket->packetNumber-receivedPacketsBaseIndex], time))
				{
					// Fill in the hole
					receivedPackets[internalPacket->packetNumber-receivedPacketsBaseIndex]=time;
				}
				else
				{
#ifdef _DEBUG
					// If you see this many times, either you forgot Sleep(0) in your main game loop so performance sucks and it is resending needlessly
					printf( "Warning: Got duplicate packet (%i).\n", internalPacket->packetNumber );
#endif
			...
			{
#ifdef _DEBUG
				if ((PacketNumberType)(internalPacket->packetNumber-receivedPacketsBaseIndex) > receivedPackets.size() + 20)
					printf("Got %i packet holes.  Probably an overflow bug.\n", (PacketNumberType)(internalPacket->packetNumber-receivedPacketsBaseIndex - receivedPackets.size()));
#endif
				//     Add 0 times to the queue until (packetNumber - baseIndex) < queue size.
				while ((PacketNumberType)(internalPacket->packetNumber-receivedPacketsBaseIndex) > receivedPackets.size())
...
int ReliabilityLayer::Receive( char **data )
{
	...
	if ( outputQueue.size() > 0 )
	{
		...
		//delete [] internalPacket->data;
		//unsigned int *blah = (unsigned int*)((unsigned char*)internalPacket->data+2);					// TODO - remove me
		//printf("ReliabilityLayer::Receive pushing Data=%i. outputQueue.size()=%i\n", *blah, outputQueue.size());
		int bitLength;
...
void ReliabilityLayer::Update( SOCKET s, PlayerID playerId, int MTUSize, unsigned int time )
{
	...
	while ( IsFrameReady( time ) )
	{
		...
		if ( updateBitStream.GetNumberOfBitsUsed() > 0 )
		{
			...
			dt->sendTime = time + 1 + ( randomMT() % 150 );
			delayList.insert( dt );
#endif
...
// reliabilityLayerMutexes[resendQueue_MUTEX].Lock();
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::InsertPacketIntoResendQueue( InternalPacket *internalPacket, unsigned int time )
{
	//reliabilityLayerMutexes[lastAckTime_MUTEX].Lock();
