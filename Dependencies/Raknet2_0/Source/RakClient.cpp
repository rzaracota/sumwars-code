...
void RakClient::InitializeSecurity( char *privKeyP, char *privKeyQ )
{
...
Packet* RakClient::Receive( void )
{
	...
	if ( packet )
	{
		...
		if ( packet->data[ 0 ] == ID_TIMESTAMP &&
			packet->length == SetRandomNumberSeedStruct_Size )
		{
			...
			if ( s.typeId != ID_SET_RANDOM_NUMBER_SEED ) return packet;
...
bool RakClient::RPC( char* uniqueID, char *data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp )
{
	if ( remoteSystemList == 0 )
		return false;
