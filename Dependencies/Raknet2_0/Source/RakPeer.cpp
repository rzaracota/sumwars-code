...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::InitializeSecurity(char *pubKeyE, char *pubKeyN, char *privKeyP, char *privKeyQ )
{
	if ( endThreads == false )
		return ;
		...
		else
		{
			...
			if ( remoteSystem != 0 )
			{
				...
				*/
				...
				RakNet::BitStream nICS_BS( NewIncomingConnectionStruct_Size );
				newIncomingConnectionStruct.Serialize(nICS_BS);
				Send( &nICS_BS, SYSTEM_PRIORITY, RELIABLE, 0, myPlayerId, false );
				return true;
			}
...
// broadcast: True to send this packet to all connected systems.  If true, then playerId specifies who not to send the packet to.
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::RPC( char* uniqueID, char *data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool shiftTimestamp )
{
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::CloseConnection( PlayerID target, bool sendDisconnectionNotification, unsigned int blockDuration )
{
...
void RakPeer::Ping( PlayerID target )
{
	...
	RakNet::BitStream pingBS( PingStruct_Size );
	ping.Serialize( pingBS );

	Send( &pingBS, SYSTEM_PRIORITY, UNRELIABLE, 0, target, false );
}
...
void RakPeer::Ping( char* host, unsigned short remotePort, bool onlyReplyOnAcceptingConnections )
{
	...
	RakNet::BitStream pingBS( UnconnectedPingStruct_Size );
	s.Serialize( pingBS );

	SocketLayer::Instance()->SendTo( connectionSocket,
		( char* ) pingBS.GetData(), pingBS.GetNumberOfBytesUsed(), ( char* ) host, remotePort );
}
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SetOfflinePingResponse( char *data, const unsigned int length )
{
...
void RakPeer::HandleConnectionRequest( PlayerID playerId, unsigned char *AESKey, bool setAESKey )
{
	...
	if ( GetNumberOfIncomingConnections() < maximumIncomingConnections )
	{
		...
		int result = SocketLayer::Instance()->SendTo( connectionSocket,
			(char *) dsBitS.GetData(), dsBitS.GetNumberOfBytesUsed(),
	...
	if ( remoteSystem )
	{
		...
		if ( remoteSystem->reliabilityLayer.HandleSocketReceiveFromConnectedPlayer( data, length ) == false )
		{
			...
			if ( !( ( unsigned char ) ( data ) [ 0 ] == ID_CONNECTION_REQUEST ||
				( ( unsigned char ) ( data ) [ 0 ] == ID_PING && length == UnconnectedPingStruct_Size ) ||
				( ( unsigned char ) ( data ) [ 0 ] == ID_PONG && length >= UnconnectedPingStruct_Size ) ||
				( ( unsigned char ) ( data ) [ 0 ] == ID_PING_OPEN_CONNECTIONS && length == UnconnectedPingStruct_Size ) ||
	...
	else
	{
		if ( ( unsigned char ) ( data ) [ 0 ] == ID_CONNECTION_REQUEST )
		{
				...
				length == 1 + sizeof( big::u32 ) + sizeof( RSA_BIT_SIZE ) + 20 )
			{
				...
				else if ( ( unsigned char ) ( data ) [ 0 ] == ID_CONNECTION_REQUEST_ACCEPTED && length == ConnectionAcceptStruct_Size )
				{
					...
					if ( requestedConnection )
					{
						...
						ConnectionAcceptStruct * cas2 = ( ConnectionAcceptStruct* )data;

						// Find a free remote system struct to use
						RakNet::BitStream casBitS(data, length, false);
						...
						if ( remoteSystem != 0 )
						{
							...
							rakPeer->Send( &nICS_BS, SYSTEM_PRIORITY, RELIABLE, 0, playerId, false );
					...
					if ( ( ( unsigned char ) ( data ) [ 0 ] == ID_PING || ( unsigned char ) ( data ) [ 0 ] == ID_PING_OPEN_CONNECTIONS )
						&& length == UnconnectedPingStruct_Size )
					{
						...
						RakNet::BitStream tempBitStream( UnconnectedPingStruct_Size );
						...
						if ( ( unsigned char ) ( data ) [ 0 ] == ID_ADVERTISE_SYSTEM && length == sizeof( unsigned char ) )
						{
							...
							if ( ( unsigned char ) ( data ) [ 0 ] == ID_PONG && length >= UnconnectedPingStruct_Size )
							{
								Packet * packet = PacketPool::Instance()->GetPointer();
								...
								// Deserialize the unconnected ping to ensure we are Endian-safe
								UnconnectedPingStruct unping;
								RakNet::BitStream unpingBS( data, UnconnectedPingStruct_Size, false );
								unping.Deserialize( unpingBS );

								unping.sendPingTime = RakNet::GetTime() - unping.sendPingTime;
								unpingBS.Reset();
								unping.Serialize(unpingBS);
								memcpy( packet->data, ( char* ) unpingBS.GetData(),
									unpingBS.GetNumberOfBytesUsed() );
	...
	for ( remoteSystemIndex = 0; remoteSystemIndex < maximumNumberOfPeers; ++remoteSystemIndex )
	{
		if ( remoteSystemList[ remoteSystemIndex ].playerId != UNASSIGNED_PLAYER_ID )
		{
			...
			while ( bitSize > 0 )
			{
				...
				if ( ( unsigned char ) data[ 0 ] == ID_PONG && byteSize == PingStruct_Size )
				{
					// Copy into the ping times array the current time - the value returned
					// First extract the sent ping
					RakNet::BitStream psBS( data, PingStruct_Size, false );
					PingStruct ps;
					ps.Deserialize(psBS);
				...
				else if ( ( unsigned char ) data[ 0 ] == ID_PING && byteSize == PingStruct_Size )
				{
					RakNet::BitStream psBS( data, PingStruct_Size, false );
					PingStruct ps;
					ps.Deserialize( psBS );
					...
					Send( &psBS, SYSTEM_PRIORITY, UNRELIABLE, 0, remoteSystem->playerId, false );
					delete [] data;
				}
				else if ( ( unsigned char ) data[ 0 ] == ID_NEW_INCOMING_CONNECTION && byteSize == NewIncomingConnectionStruct_Size )
				{
					Ping( remoteSystem->playerId );
