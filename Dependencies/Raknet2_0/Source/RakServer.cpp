...
void RakServer::InitializeSecurity( char *pubKeyE, char *pubKeyN )
{
...
bool RakServer::Start( unsigned short AllowedPlayers, unsigned int connectionValidationInteger, int threadSleepTimer, unsigned short port )
{
	bool init;
	...
	if ( RakPeer::IsActive() && synchronizedRandomInteger )
	{
		...
		if ( time > nextSeedUpdate || ( packet && packet->data[ 0 ] == ID_NEW_INCOMING_CONNECTION ) )
		{
			...
			if ( packet && packet->data[ 0 ] == ID_NEW_INCOMING_CONNECTION )
				Send( &s_BitS, SYSTEM_PRIORITY, RELIABLE, 0, packet->playerId, false );
			else
				Send( &s_BitS, SYSTEM_PRIORITY, RELIABLE, 0, UNASSIGNED_PLAYER_ID, true );
		}
	}
...
bool RakServer::RPC( char* uniqueID, char *data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool shiftTimestamp )
{
...
void RakServer::SetOfflinePingResponse( char *data, const unsigned int length )
{
