...
void Action::updateTranslation()
{
	...
	a->m_description = gettext("The archer has made a pact with the spirits of wind and ice. His resistance against wind- and icedamage is increased by 30 percent.");
	
	a = &(Action::m_base_info[Action::RESIST_AIR]);
	a->m_name = gettext("North Hunter");
	a->m_description = gettext("The archers good longterm relations with the spirits of wind and ice has caused his ");
