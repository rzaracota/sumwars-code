...
void TemplateLoader::searchObjectGroupTemplate(TiXmlNode* pParent, std::list<ObjectGroupTemplate*> &object_group_template_list, std::list<std::string> &name_list)
{
	...
	case TiXmlNode::TEXT:
	{
		std::cout << pParent->Parent()->Value() << "<<<<<<" << std::endl;
		if (!strcmp(pParent->Parent()->Value(), "ObjectContent"))
		{
			pText = pParent->ToText();
			const char* eintext = pParent->Value();
			printf( "Text inside: [%s]\n", pText->Value() );
		}
		
		/*TiXmlAttribute* pAttrib=pParent->Parent()->ToElement()->FirstAttribute();
		std::cout << pAttrib << std::endl;
		if (pAttrib)
			std::cout << "==>" << pAttrib->Value() << std::endl;*/
		//pAttrib=pParent->Parent()->Parent()->ToElement()->FirstAttribute();
		//std::cout << pAttrib << std::endl;

		/*if (!strcmp(pParent->Parent()->Value(),"ObjectContent"))
		{
			while (!strcmp(pParent->Parent()->Value(),"ObjectContent") && pAttrib)
			{
				if (!strcmp(pAttrib->Name(), "name"))
					std::cout << "==>" << pAttrib->Value() << std::endl;
