...
bool Application::init(char *argv)
{
	...
	if (!PHYSFS_exists(ogreCfgUser.c_str()) && !PHYSFS_exists(ogreCfgSystem.c_str()))
	{
		// we are on Linux so we can safely assume OpenGL render system
		// we won't be making any resolution assumptions, Ogre will figure out native resolution
		// and start with that
		std::string str =
	...
	if (!PHYSFS_exists(pluginsCfgUser.c_str()) && !PHYSFS_exists(pluginsCfgSystem.c_str()))
	{
		std::string str =
			"# This file's template has been autogenerated using cmake (see src/gui/config.h.in)\n"
