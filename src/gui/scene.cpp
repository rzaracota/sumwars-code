...
void Scene::createScene()
{
	...
	if (region !=0)
	{
		...
		m_terrainOptions = OGRE_NEW Ogre::TerrainGlobalOptions();

		m_terrainGroup = OGRE_NEW Ogre::TerrainGroup(m_scene_manager, Ogre::Terrain::ALIGN_X_Z, 65.0f, 128.0f);
...
{
    // Configure global
    m_terrainOptions->setMaxPixelError(8);
    // testing composite map
    m_terrainOptions->setCompositeMapDistance(3000);

    // Important to set these so that the terrain knows what to use for derived (non-realtime) data
    m_terrainOptions->setLightMapDirection(light->getDerivedDirection());
    m_terrainOptions->setCompositeMapAmbient(m_scene_manager->getAmbientLight());
    m_terrainOptions->setCompositeMapDiffuse(light->getDiffuseColour());

    // Configure default import settings for if we use imported image
    Ogre::Terrain::ImportData& defaultimp = m_terrainGroup->getDefaultImportSettings();
    defaultimp.terrainSize = 65;
    defaultimp.worldSize = 128.0f;
    defaultimp.inputScale = 10.0f;
    defaultimp.minBatchSize = 33;
    defaultimp.maxBatchSize = 65;
    // textures
    defaultimp.layerList.resize(3);
    defaultimp.layerList[0].worldSize = 15;
    defaultimp.layerList[0].textureNames.push_back("grass1.png");
    defaultimp.layerList[0].textureNames.push_back("grass1.png");
	defaultimp.layerList[1].worldSize = 15;
    defaultimp.layerList[1].textureNames.push_back("rockwall.tga");
    defaultimp.layerList[1].textureNames.push_back("rockwall.tga");
	defaultimp.layerList[2].worldSize = 15;
    defaultimp.layerList[2].textureNames.push_back("cave_ground.png");
    defaultimp.layerList[2].textureNames.push_back("cave_ground.png");
}

void Scene::defineTerrain(long x, long y)
{
    ...
    else
    {
		...
		float *temp = m_document->getLocalPlayer()->getRegion()->getHeight()->ind(0,0);
		m_terrainGroup->defineTerrain(x,y,m_document->getLocalPlayer()->getRegion()->getHeight()->ind(0,0));
        //m_terrainGroup->defineTerrain(x, y, &img);
		//m_terrainGroup->defineTerrain(x, y, "PageX0Y0.f32");
